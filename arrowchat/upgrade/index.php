	...
	*/
	...
	if (isset($_POST['upgrade']) || isset($_POST['upgrade_x'])) 
	{
		...
		$result = $db->execute("
			...
			VALUES ('max_upload_size', '5', '0'), ('enable_moderation', '0', '0'), ('chatroom_transfer_on', '0', '0'), ('chatroom_message_length', '150', '0'), ('push_ssl', '0', '0')
		");
		...
		$result = $db->execute("
			...
			ADD `description` varchar(100) collate utf8_bin NOT NULL AFTER `name`,
			ADD `welcome_message` varchar(255) collate utf8_bin default '' AFTER `description`,
			ADD `image` varchar(100) collate utf8_bin NOT NULL AFTER `welcome_message`,
		...
		$result = $db->execute("
			ALTER TABLE `arrowchat_chatroom_users`
			ADD `silence_length` int(3) unsigned NOT NULL AFTER `block_chats`,
			ADD `silence_time` int(15) unsigned NOT NULL AFTER `silence_length`
		");
