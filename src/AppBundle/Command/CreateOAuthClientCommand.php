...
class CreateOAuthClientCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $customers = $container->get('doctrine')->getRepository('MultiservicesArxisBundle:Usuario')->findAll();
        ...
        foreach ($customers as $customer) {
            ...
            $output->writeln(sprintf("<info>Usuario <comment>%s</comment> linked to client <comment>%s</comment></info>",
                $customer->getId(),
                $client->getName()));
        }
    }
