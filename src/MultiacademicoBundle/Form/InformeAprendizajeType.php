...
class InformeAprendizajeType extends AbstractType
{
    private $q;
    private $p;
            ...
            ->addEventListener(FormEvents::PRE_SET_DATA, function (FormEvent $event) {
                ...
                if ($dataparent instanceof CursoACalificar)
                {
                    ...
                    if($this->q<3)
                    {
                        ...
                        $recvar='q'.$this->q.'P'.$this->p.'Recomendacion';
                        $form->add($recvar, TextareaType::class, ['required'=>false]);
                        $planmejoravar='q'.$this->q.'P'.$this->p.'Planmejora';
                        $form->add($planmejoravar, TextareaType::class, ['required'=>false]);
                    }
                    if ($this->p==3)
                       {
                        $recvarq='q'.$this->q.'Recomendacion';
                        $form->add($recvarq, TextareaType::class, ['required'=>false]);
                        $planmejoravarq='q'.$this->q.'Planmejora';
                        $form->add($planmejoravarq, TextareaType::class, ['required'=>false]);
                       }      
                }  
