 ...
 *  Lingua latД«na    ['la']
 */
appConfig.voice_command_lang = 'es-ES';
/*
...
define('app',["angular","angular-couch-potato","angular-ui-router","angular-animate","angular-bootstrap"],function(a,b){var c=a.module("app",["ngSanitize","ngAnimate","scs.couch-potato","ui.router","oc.lazyLoad","ui.bootstrap","angular-loading-bar","BlankonCtrl","smart-templates"]);return b.configureApp(c),c.factory("settings",["$rootScope",function(a){var b="http://localhost/multicad",c={baseURL:b,pluginPath:"../vendor",pluginCommercialPath:b+"/assets/commercial/plugins",globalImagePath:"img",adminImagePath:b+"/assets/admin/img",cssPath:"styles",jsPath:"scripts",dataPath:"data",additionalPath:b+"/assets/global/plugins/bower_components"};return a.settings=c,c}]).config(["cfpLoadingBarProvider",function(a){a.includeSpinner=!0}]),c.config(["$provide","$httpProvider","$locationProvider",function(a,b,c){c.html5Mode(!0),b.defaults.headers.common["X-Requested-With"]="XMLHttpRequest",a.factory("ErrorHttpInterceptor",["$q",function(a){function b(a){console.log(a)}return{requestError:function(c){return b(c),a.reject(c)},responseError:function(c){return b(c),a.reject(c)}}}]),b.interceptors.push("ErrorHttpInterceptor")}]),c.run(["$couchPotato","$rootScope","$state","$stateParams",function(a,b,d,e){c.lazy=a,b.$state=d,b.$stateParams=e}]),c});
define('scripts/controllers',["app"],function(a){"use strict";function b(a){a.supportIE=1}return b.$inject=["$scope"],a.controller("BlankonCtrl",b)});
define('smart-templates',["angular"],function(){angular.module("smart-templates",[]).run(["$templateCache",function(a){}])});
define('includes',["scripts/controllers","smart-templates"],function(){"use strict"});
window.name="NG_DEFER_BOOTSTRAP!",define('main',["require","jquery","angular","domReady","bootstrap","appConfig","app","includes"],function(a,b,c,d){"use strict";d(function(a){c.bootstrap(a,["app"]);c.resumeBootstrap(),console.log("aqui paso algo ya")})});
